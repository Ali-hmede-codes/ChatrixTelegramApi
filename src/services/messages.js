...
const { detectArabic } = require("../utils/arabicDetection");
...
function formatMessage(msg) {
  ...
  const arabicResult = detectArabic(rawText);

  const result = {
    id: Number(msg.id),
    date: Number(msg.date),
    text,
    isArabic: arabicResult.isArabic,
    arabicConfidence: Math.round(arabicResult.confidence * 100),
